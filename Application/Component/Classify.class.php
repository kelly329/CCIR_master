...
class Classify
{
    ...
    static public function tree($data, $pid = 0, $count = 1)
    {
        foreach ($data as $key => $value) {
            if ($value['pid'] == $pid) {
                ...
                self::$treeList [] = $value;
                unset($data[$key]);
                self::tree($data, $value['privilegeid'], $count + 1);
            }
        }
     ...
     * @return Array $treeList
     */
    static public function typetree($data, $pid = 0, $count = 1)
    {
        foreach ($data as $key => $value) {
            if ($value['pid'] == $pid) {
                $value['level'] = $count;
                self::$treeList [] = $value;
                unset($data[$key]);
                self::tree($data, $value['typeid'], $count + 1);
            }
        }
        return self::$treeList;
    }
    ...
    static public function getAllNode($data, $id)
    {
        foreach ($data as $key => $value) {
            if ($value['pid'] == $id) {
                self::$nodeList [] = $value;
                unset($data[$key]);
                self::getAllNode($data, $value['privilege']);
            }
            if ($value['id'] == $id) {
                self::$nodeList [] = $value;
            }
    ...
    {
        foreach ($data as $key => $value) {
            if ($value['privilegeid'] == $fatherid) {
                self::$fatherNodeList [] = $value;
                unset($data[$key]);
                self::getAllFaNode($data, $value['pid']);
            }
        }
